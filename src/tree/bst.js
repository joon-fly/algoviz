...
export function drawBST(ctx, W, H, snapshot) {
  ...
  // Edges
  snapshot.edges.forEach(({parentId,childId}) => {
    const p=nodeMap[parentId], c=nodeMap[childId];
    if (!p||!c) return;
    ctx.beginPath();
    ctx.moveTo(p.x,p.y); ctx.lineTo(c.x,c.y);
    ctx.strokeStyle='#2a2845'; ctx.lineWidth=1.5;
    ctx.stroke();
  });
  ...
  snapshot.nodes.forEach(node => {
    ...
    const isActive = hk==='visiting'||hk==='found'||hk==='inserted';

    if (isActive) {
      ctx.beginPath(); ctx.arc(node.x,node.y,NODE_R+7,0,Math.PI*2);
      ctx.fillStyle=hk==='found'?'rgba(106,247,200,0.15)':
                    hk==='inserted'?'rgba(124,106,247,0.15)':
