...
// ── Boot ──────────────────────────────────────────────────────────
// Support graph.html#dijkstra hash links from index.html
const hashAlgo = window.location.hash.replace('#', '');
if (hashAlgo && META[hashAlgo]) {
  currentAlgo = hashAlgo;
  document.querySelectorAll('.av-algo-btn').forEach(b => {
    b.classList.toggle('active', b.dataset.algo === hashAlgo);
  });
}
updateInfoPanel(currentAlgo);
updatePseudo(-1);
