...
function updateInfoPanel(algo) {
  ...
  // Goal row visibility
  const needsGoal = algo==='astar';
  $('goalRow').classList.toggle('hidden', !needsGoal);
}
...
$('algoList').addEventListener('click', e => {
  ...
  const prevWeighted = META[currentAlgo].weighted;
  currentAlgo = btn.dataset.algo;
  updateInfoPanel(currentAlgo);
  if (graph.nodes.length) {
    // If weighted setting changed, regenerate graph edges; otherwise just re-run algorithm
    if (META[currentAlgo].weighted !== prevWeighted) {
      generateAll();
    } else {
      buildSnapshots();
    }
